...
listar()
function listar(){
    ajax.open("GET","http://"+window.location.hostname+":8080/usuario/buscarprds");
...
function incluir(){
    ...
    ajax.open("POST","http://"+window.location.hostname+":8080/adm/addprd");
...
function editarconfirm(){
    ...
    ajax.open("PUT","http://"+window.location.hostname+":8080/adm/atualizarprd");
    ajax.setRequestHeader("Authorization", sessionStorage.getItem('token'));
    ajax.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
...
function apagarconfirm(){
    ...
    ajax.open("DELETE","http://"+window.location.hostname+":8080/adm/deletarprd/"+idApgr);
    ajax.setRequestHeader("Authorization", "Bearer "+token);
    ajax.send();
    ajax.onload = function(){
        if(this.status == 200){
